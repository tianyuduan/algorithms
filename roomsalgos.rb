...
Input: [[1,3],[3,0,1],[2],[0]]
def keyFinderRoom(nestedArray)
	keyCollection = [0]
	keyCollectioLength = keyCollection.length

	While true . //conditional
	keyCollection.each do |j|  //n
  		keyCollection << nestedArray[j] if uniq?  //n
   	 End

 If KeyCollection.length == keyCollectionLength //1
	Break

 End
keyCollectionLength= KeyCollection.length  // 1
End

If Keycollection.length == nestedArray.length //1
